...
def SetupTileMap():
    global tiles, tilesetimage, tilesetname, indoortilesetname, waterTiles, waterObjectCount
    ...
    tiles = [None] * (tilecount+waterObjectCount)
    ...
    waterTiles = [None] * 8 * waterObjectCount
    for x in range(8):
        for y in range(waterObjectCount):
            image = image1.crop((x*16,y*16,x*16+16,y*16+16))
            waterTiles[x+8*y] = pygame.image.fromstring(image.tobytes(),image.size,image.mode)
    for x in range(waterObjectCount):
        tiles[128+x] = waterTiles[8*x]
...
def draw():
    global txtemp, tytemp, numberToPaint, paintWidth, paintHeight, sizeX, sizeY, editMode, copyingMap, copyingTypeMap, zonedata, waterTiles, waterTileIndex, waterObjectCount
    ...
    waterTileIndex+=1
    if(waterTileIndex == 160):
        waterTileIndex = 0
    for x in range(waterObjectCount):
        tiles[128+x] = waterTiles[8*x+int(waterTileIndex/20)]
    ...
    if editMode == 0:
        if(not copyingMap):
            ...
            for x in range(waterObjectCount):
                screen.blit(tiles[128+x], ((sizeX*16)+(16*x)+16,240))
        ...
        if(tx > (sizeX) and ty > 6 and ty < 16 and not copyingMap):
            if pygame.mouse.get_pressed()[0]:
                if(shift):
